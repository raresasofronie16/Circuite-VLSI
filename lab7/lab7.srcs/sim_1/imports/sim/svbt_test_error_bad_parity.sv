`include "svbt_include.sv"

constraint svbt_packet::keep_flag_mismatched_parity_1 { flag_mismatched_parity==1; }

//LAB: implement a test that sends packets with bad parity
//remember to use 'constraint_mode(0)' for the constraints you want to disable

program test;
   svbt_environment env;
   
   initial 
   begin
     env=new(15," Environment",25); //11001
     env.data_in_generator.curent_packet.keep_flag_mismatched_parity_0.constraint_mode(0);
     env.run();
   end
endprogram
