...
class svbt_environment extends svbt_base_unit;
   ...
   task testend();
      ...
      while(1)
      begin
        if(scbd.total_packets == number_of_packets)
        begin
            scbd.check_empty();
            if(scbd.errors)
            begin
                $display("\n FAILED , nr de erori: %d", scbd.errors);
            
            end
            else
            $display("\n PASSED , , nicio erorare");
            
            $finish(1);
        end
        else if(counter == watchdog) 
        begin
            scbd.check_empty();
            $display("EROARE din cauza watchodu-ului, nr de erori : %d", scbd.errors);
            $finish(1);
        
        end
        
        else begin @(posedge top.clock); 
            counter++;
