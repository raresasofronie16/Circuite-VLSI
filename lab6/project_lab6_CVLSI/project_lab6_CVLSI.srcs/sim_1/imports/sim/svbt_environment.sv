...
class svbt_environment extends svbt_base_unit;
	...
	//use inspiration from BFM
     for(int i=0; i<3; i++) begin
      ...
      //LAB: instantiate the scoreboard	
        scoreboard = new("SCOREBOARD", 99, data_in_mbox, channel_out_mbox, top.input_intf.rcv);
   ...
   task run();
      ...
      begin
        $display("[%0t ]Test trecut, 0 erori", $time);
        
      end
      else
      begin
        $display("[%0t]Test picat, %d erori gasite", $time, scoreboard.errors);
